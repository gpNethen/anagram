...
// This program takes two lines of input from the user through cin and
// Checks to see if the two lines of input are anagrams. Please advise: The 
// Anagram check only pays attention to LETTERS and will ignore numbers, 
// Special characters, as well as white spaces.
...
void countLetters(char input[], int inputSize, int output[], int outputSize){ //first function definition
	...
	for(i=0; i < inputSize; i++){     // for loop to check each position in the input arrays for each letter only
		//cout << input[i];           // Test point: outputs contents of each index to verify correctness [not necessary for main program]
		switch (input[i]){            // Checks each position of the input array and, for each case, increments the associated position
			case 'A':                 // In the letter counting int array
			case 'a':                 // Example: output[0] == 'a' counter > output[1] == 'b' counter > - - - - > output[25] == 'z' counter
				output[0]+=1;
				break;
			case 'B':
			case 'b':
...
int main()                                      // Beginning of main program
{
    ...
    cout << "Please enter your first line below [Max 60 characters]:" << endl << "     ";  // The user
    ...
    cout << "Please enter your second line below [Max 60 characters]:" << endl << "     "; // More instruction
...
/*  Start of debug tests (to be commented out in final product)[Not necessary for main program to succeed]
